...
import { Tasks, Schedule } from '@/composables/lib/respone'
...
const { queryTasks, querySchedules } = useAPI()
const { authData } = useAuth()

const { data: tasksData } = useQuery({
  queryKey: ['calendar-tasks'],
  queryFn: () => queryTasks({ user_id: authData.value?.user_id }),
  select: (data) => data.data,
})

const { data: schedulesData } = useQuery({
  queryKey: ['calendar-schedules'],
  queryFn: () => querySchedules({ user_id: authData.value?.user_id }),
  select: (data) => data.data as any as Schedule[],
})

const events = computed<Event[]>(() => {
  const taskEvents = (tasksData.value || [])
    .map((task: Tasks) => ({
      id: `task-${task.id}`,
      date: task.deadline ? new Date(task.deadline).toISOString().slice(0, 10) : '',
      title: task.title,
      type: 'deadline',
  ...
  const scheduleEvents = (schedulesData.value || []).map((schedule: Schedule) => ({
...
const calendarDays = computed(() => {
  ...
  for (let i = 0; i < firstDay; i++) {
    daysArr.push(new Date(currentYear.value, currentMonth.value, i - firstDay + 1))
  }
  // Current month
  ...
  while (daysArr.length < 42) {
    daysArr.push(
      new Date(currentYear.value, currentMonth.value + 1, daysArr.length - numDays - firstDay + 1)
    )
  }
  return daysArr
})
...
const selectedDate = ref(new Date(currentYear.value, currentMonth.value, today.getDate()))
...
<template>
  <div
    class="h-full bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 min-h-screen p-8 max-w-screen"
    style="margin-top: 21px">
    <div class="mx-auto">
      <!-- Header -->
      <div class="flex items-center justify-between mb-6" style="margin-bottom: 12px">
        <div class="flex items-center gap-x-4">
          <Button icon="pi pi-angle-left" class="p-button-text" @click="prevMonth" />
          <h2 class="text-2xl font-semibold text-white">{{ monthYear }}</h2>
          <Button icon="pi pi-angle-right" class="p-button-text" @click="nextMonth" />
        </div>
      ...
      <!-- Calendar Grid -->
      <div v-if="view === 'month'" class="bg-white rounded-xl shadow p-4 text-black mt-2">
        <div class="grid grid-cols-7 border-b text-center text-gray-500 font-medium">
      ...
      <div v-else-if="view === 'week'" class="bg-white rounded-xl shadow p-4 text-black mt-2">
        <div class="grid grid-cols-8 border-b text-center text-gray-500 font-medium">
          <div class="py-2">Hour</div>
          <div v-for="d in weekDays" :key="d.toISOString()" class="py-2">
            {{
        ...
        <div>
          <div v-for="hour in dayHours" :key="hour" class="grid grid-cols-8 border-t min-h-[32px]">
            <div class="text-xs text-gray-400 flex items-center justify-center">
              {{ hour }}
            </div>
            <div v-for="d in weekDays" :key="d.toISOString()" class="relative px-1">
              <template v-for="event in eventsForWeekDay(d, hour)" :key="event.id">
                <div
                  :class="[
      ...
      <div v-else-if="view === 'day'" class="bg-white rounded-xl shadow p-4 text-black mt-2">
        ...
        <div>
          <div v-for="hour in dayHours" :key="hour" class="border-t min-h-[32px] flex items-center">
            <div class="w-16 text-xs text-gray-400 flex-shrink-0 text-center">
              {{ hour }}
            </div>
            <div class="flex-1 px-1">
              <template v-for="event in eventsForDayHour(selectedDate, hour)" :key="event.id">
                <div
                  :class="[
                    'rounded px-2 py-1 text-xs flex items-center gap-1 my-0.5',
                    event.type === 'meeting' ? 'bg-gray-100 text-gray-700' : '',
      ...
      <!-- Legend -->
      <div class="flex items-center rounded-xl p-4 gap-x-3.5" style="margin-top: 12px">
        ...
        <div class="flex items-center gap-x-2">
          <span class="w-4 h-4 rounded border border-blue-200 bg-blue-100 inline-block"></span>
